...
export default function ApplyPage() {
  ...
  // 🔹 URL ?code=직원코드 OR 쿠키 ref 둘 다 지원
  useEffect(() => {
    const cookieCode = getRefFromCookie();
    const urlCode = searchParams.get("code");
    // URL 코드 있으면 그걸 최우선으로 적용
    let finalCode = urlCode || cookieCode || null;

    // URL에 code 있으면 쿠키도 그걸로 덮어쓰기
    if (urlCode) {
    ...
    >
      ...
      >
        ...
        {refEmployee ? (
          ...
          <div>담당 직원 정보가 없습니다. (직원 전용 링크가 아닌 경로로 들어온 경우)</div>
        )}
      </div>
